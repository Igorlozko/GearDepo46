...
// acts like a wrapper protecting the unauthorized access 
const Protected = ({children}) => {
    const {state: {currentUser}} = useValue() // checks if the current user is in the global state 
  return (
    currentUser ? children : <ProtectMessage/> // if there is a user presetn then show the children = componments otherwise show message 
  )
}
